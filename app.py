...
def movies():
    ...
    return render_template('movies.html', wishlist=wishlist, reviews=reviews)
...
@app.route('/get_show_recommendations')
def get_show_recommendations():
    if 'username' not in session:
        return jsonify({"status": "error", "message": "Not logged in"})
    
    recommendations = get_show_recommendations(session['username'])
    return jsonify({"status": "success", "recommendations": recommendations})
